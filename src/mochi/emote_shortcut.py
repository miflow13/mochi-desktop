"""Privacy-safe bridge for Mochi's user-facing emote catalogue shortcut."""
...
class GnomeShellEmoteCatalogueShortcutBackend:
    """Receive a zero-payload emote-catalogue request from GNOME Shell."""

    name = "GNOME Shell emote catalogue shortcut"
    ...
    SIGNAL_NAME = "EmoteCatalogueRequested"
    ...
    def start(self, on_requested: Callable[[], None]) -> bool:
        ...
        try:
            ...
            if not subscription_id:
                self.last_error = "emote catalogue signal subscription failed"
                self.stop()
                return False
...
class EmoteCatalogueShortcutMonitor:
    """Lifecycle wrapper for the user-facing catalogue shortcut bridge."""
    ...
    def __init__(
        ...
        *,
        ...
        backend: GnomeShellEmoteCatalogueShortcutBackend | None = None,
    ) -> None:
        self._on_requested = on_requested
        self._logger = logger or logging.getLogger(__name__)
        self._backend = backend or GnomeShellEmoteCatalogueShortcutBackend()
    ...
    def start(self) -> bool:
        ...
        if self.available:
            self._logger.info("Emote catalogue shortcut enabled via %s", self._backend.name)
        else:
            self._logger.debug(
                "Emote catalogue shortcut unavailable via %s: %s",
                self._backend.name,
